...
const Whack = () => {
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            ...
            const evenIndex = Math.floor(Math.random() * (initialState.length / 2)) * 2;
            const newDivs = [...divs];
            newDivs[randomIndex] = {
                ...newDivs[randomIndex],
    ...
    return (
        <Container color={"#29215A"}>
            {start && <div className={classes.whack}>
                <div className={classes.data}>
                    <div className={classes.score}>Score is : {score}</div>
                    <div className={classes.instructions}>מה התרגום של המילה "{initialState[next + 1].content}" באנגלית </div>
                </div>
                    ...
                    <div className={classes.holes}>
                        {divs.map((div, index) => (
                            <div
                                ...
                                style={{
                                    border: 'none',
                                    // backgroundImage: index === randomDivIndex ? `url(${mole})` : 'none',
                                ...
                                {index === randomDivIndex && <img src={mole} alt="mole" />}
                                {div.content}
                            </div>
                        ))}
                    </div>
            ...
            {!start && <div className={classes.start}>
                <div className={classes.btn}>
                    <h3>"המשחק הוא "הכה החפרפרת </h3>
                    <h3>כדי לתרגל את המילים שלמדנו בכל פעם תתבקשו לפגוע בחפרפרת עם התרגום </h3>
                    <h3> פגיעה נכונה תזכה ב20 נקודות</h3>
                    <h3>פגיעה לא נכונה וירדו 5 נקודות מהציון </h3>
                    <h3>בהצלחה</h3>
                    <span onClick={() => { setStart(true) }} >התחל משחק</span>
                </div>
            </div>}
